  ...
  function installNodePackage (dependency) {
    return new Promise(function (resolve, reject) {
      try {
        ...
        var f = fs.statSync(path.join(dependency.location, 'package.json'))
        if (f.isFile()) {
          whenVerbose('install: installing npm dependencies:')
          shelljs.exec(
          ...
          whenVerbose('install: package.json is not a file')
        }
      } catch (err) {
