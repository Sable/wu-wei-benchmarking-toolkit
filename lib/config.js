...
exports.validateDescription = (function () {
  ...
  return function (descriptionPath) {
...
exports.createRunner = function (config, parsed) {
  ...
  config.compiler.commands.forEach(function (c, i) {
    ...
    fs.writeFileSync(path.join(shelljs.pwd(), 'log-' + i + '.txt'), status.stdout)
  ...
  fs.writeFileSync(path.join(shelljs.pwd(), 'build.json'), JSON.stringify(config, null, '  '))
  shelljs.popd()
  shelljs.config.silent = silentState
}
...
exports.determinePlatform = function (config, options) {
  ...
  if (options.platform) {
    ...
    var currentPlatform = JSON.parse(status.stdout)
