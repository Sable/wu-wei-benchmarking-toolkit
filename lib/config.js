...
function getConfigFromFileSystem (dirRoot, cb) {
  ...
  function keepConfigPaths (paths) {
    ...
    return paths.filter(function (file) {
      var isHiddenDir = (file.match('^\\..*$'))
      var isNodeModules = (file.match('^.*node_modules.*$'))
      return file.match(regex) && !isHiddenDir && !isNodeModules
    })
  }
...
exports.executeRun = function (config, options) {
  ...
  if (!options['skip-output-verification'] &&
    ...
    } else if (JSON.stringify(jsonOutput.output) !== JSON.stringify(expectedOutput)) {
      console.log("Invalid output of " + JSON.stringify(jsonOutput.output) + " expected " + JSON.stringify(expectedOutput) + " instead")
      process.exit(1)
